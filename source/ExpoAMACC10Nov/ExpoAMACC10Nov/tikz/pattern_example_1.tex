\begin{tikzpicture}[scale=.4,font=\small,anchor=center]
	...
	\begin{scope}
		...
		\foreach \y in {-2,0,2,4}
		{
			...
			{				
				...
				\fill[black!40]	(\x+1,\y+0) rectangle +(1,1);
				\fill[green!40] 		(\x+1,\y+1) rectangle +(1,1);
				
				\draw	(\x+0.5,\y+0.5) node{$0$};
				\draw	(\x+0.5,\y+1.5) node{$1$};
				\draw	(\x+1.5,\y+0.5) node{$2$};
		...
		\draw[ultra thick, black, ->]	(0,2) rectangle +(2,2);
		\draw[ultra thick, black, ->]	(1,0) rectangle +(2,2);
